...
I focus on the *post-mortem* analysis of execution trace, which contains information about the events and the resources of the application. The visualisation methods I propose are based on data and visual aggregations, in order to reduce the trace complexity and provide scalable representation of the trace, in particular when its volume is huge (up to several terabytes, several billions of events). Theses methods are efficient to detect anomalies in multimedia applications executed on embedded systems, but also in parallel applications performing scientific computing on grids.  
